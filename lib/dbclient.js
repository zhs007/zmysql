...
class DBClient{
    ...
    // 返回true，表示把回调抛给上层逻辑
    __onProcErr(err) {
        if (err.code == 'ECONNRESET' || err.code == 'ECONNREFUSED' || err.code == 'PROTOCOL_CONNECTION_LOST' || err.code == 'PROTOCOL_ENQUEUE_AFTER_FATAL_ERROR' || err.code == 'ETIMEDOUT') {
            let self = this;
    ...
    __query(sql, funcOnQuery) {
        if (sql == undefined) {
            if (funcOnQuery != undefined) {
                funcOnQuery({code: 'ER_EMPTY_QUERY', message: 'Query was empty(ER_EMPTY_QUERY)'}, undefined, undefined);
            }
